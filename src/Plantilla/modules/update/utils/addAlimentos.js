...
export function generarOpcionesAlimentos(alimentos, alimentoSeleccionado = null) {
  ...
  // ⭐ Detectar ID aunque venga como objeto completo del backend
  if (alimentoSeleccionado) {
    ...
    else if (alimentoSeleccionado.id_alimento) {
  ...
  alimentos.forEach(alimento => {
    ...
    const selected = (alimento.id_alimento == idSeleccionado) ? "selected" : "";

    opciones += `<option value="${alimento.id_alimento}" ${selected}>${alimento.nombre}</option>`;
  });
